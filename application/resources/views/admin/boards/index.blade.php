        ...
        <div class="dzenkit-basic-card dzenkit-fulltable-maxwid-lg">
            ...
            <div class="dzenkit-table-header hdr-box d-flex align-items-center justify-content-between">
                ...
                <div class="d-flex align-items-center">
                    ...
                    <a href="{{ route('admin.boards.create') }}" class="btn btn-dzenkit-action"><i class="icon-plus"></i>Добавить</a>
                </div>
            </div>
            ...
            <div class="dzenkit-basic-card-body rows_super_list">
                @foreach($boards as $row)
                    <form style="display:none;" class="delete_{{ $row->id }}" action="{{ route('admin.boards.destroy', $row->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
                ...
                @endforeach
                <form class="delete_form_all" action="{{ route('admin.boards.delete_all') }}" method="GET">
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            <tbody >
                            @foreach($boards as $row)
                                <tr data-url-change="{{ route("admin.boards.change",$row->id) }}" data-row-id="{{ $row->id }}" data-sort="{{ $row->sort }}">
                                    <td>
                                            ...
                                            <div class="nav-bar d-flex align-items-center">
                                                ...
                                                <div class="status">
                                                    <div class="form-check form-switch d-flex align-items-center">
                                                        <input class="form-check-input row_is_enable" data-route="{{ route("admin.boards.toogle",$row->id) }}" type="checkbox" @if($row->enable==1) checked @endif >
                                                    </div>
                                                </div>
                                                <!-- action menu -->
                                                <div class="action">
                                                    <div class="dropleft">
                                                        ...
                                                        <ul class="dropdown-menu">
                                                            <li><a class="icon-pencil d-flex align-items-center" href="{{ route('admin.boards.show', $row->id) }}" title="show">
                                                                    Просмотр
                                                                </a></li>
                                                            <li><a class="icon-pencil d-flex align-items-center" href="{{ route('admin.boards.edit', $row->id) }}" title="show">
                                                                    Редактирование
                                                                </a></li>
                                                            <li><a class="icon-clone d-flex align-items-center" href="{{ route("admin.boards.clone",$row->id) }}">Клонировать</a></li>
